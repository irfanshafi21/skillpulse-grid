...
// Invalid payloads must stop before any database write.
for (const path of ['/api/validations/demo-check', '/api/mirror/demo-check']) {
  const response = await fetch(`${base}${path}`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}',
  })
  assert.equal(response.status, 400, `${path} must reject invalid payloads`)
  console.log(`PASS POST ${path}: invalid payload rejected`)
}
